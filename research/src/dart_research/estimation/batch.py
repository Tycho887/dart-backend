"""Research post-pass estimator using the shared measurement model."""
...
def fit_batch(
    ...
    *,
    ...
    if selected is MeasurementMode.DOPPLER_PHASE and not any(
        item.phase_rad is not None for item in usable
    ):
    ...
    fitted_jacobian = jacobian(best.x)
    f_scale = config.robust_scale_doppler_hz / config.doppler_std_hz
    ...
    if config.robust_loss == "linear":
        ...
        weights = np.ones_like(raw_residual)
    information = fitted_jacobian.T @ (weights[:, None] * fitted_jacobian)
    ...
    scaled_jacobian = np.sqrt(weights)[:, None] * fitted_jacobian * parameter_scale
