"""Research mean-element fits with optional synthetic phase observations."""
...
def rebuild_tle_mean_elements(tle, mean_anomaly_rad: float, mean_motion_rad_min: float):
    """Return a Satkit TLE with only mean anomaly and mean motion replaced."""
...
def fit_mean_elements(
    ...
    *,
    ...
    def channel_residuals(state: np.ndarray) -> tuple[np.ndarray, np.ndarray | None]:
        ...
        predicted = model.predict_many(candidate_context, prediction_state, mode, cache)
        ...
        phase = wrap_angle_rad(predicted[:, 1] + phase_biases[pass_index] - measured_phase)
    ...
    def jacobian(state: np.ndarray) -> np.ndarray:
        ...
        for index, step in enumerate(steps):
            ...
            difference = residual(right) - residual(left)
            if phase_capable:
                start, stop = len(usable), 2 * len(usable)
                difference[start:stop] = (
                    wrap_angle_rad(difference[start:stop] * config.phase_std_rad)
    ...
    baseline = np.array(
        [*initial_orbit] + [0.0] * pass_count + ([0.0] * pass_count if phase_capable else [])
    )
    sampler = qmc.LatinHypercube(d=dimension, seed=42)
    candidates = np.vstack((baseline, qmc.scale(sampler.random(config.qmc_samples), lower, upper)))
    ...
    result = least_squares(
        ...
        x_scale=np.array(
            [0.05, 0.001] + [2_000.0] * pass_count + ([1.0] * pass_count if phase_capable else [])
        ),
        ftol=1e-11,
        xtol=1e-11,
        gtol=1e-11,
    ...
    return MeanElementFit(
        ...
        phase_biases_rad=(np.asarray(result.x[2 + pass_count :]).copy() if phase_capable else None),
        covariance=covariance,
        residuals_hz=observation_residual,
        phase_residuals_rad=(None if phase_residual is None else np.asarray(phase_residual).copy()),
