...
import numpy as np
from dart.frames import tle_positions_itrf

from ..data.forest import ForestPass
from ..data.gps import GPSReference
...
def _online_position_errors_km(tle, gps: GPSReference, estimates: list) -> np.ndarray:
    ...
    result = []
    for epoch, truth in zip(gps.utc_s, gps.position_itrf_m, strict=True):
