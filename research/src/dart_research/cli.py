...
import numpy as np
import satkit as sk
from dart.frames import relative_geometry_from_state, tle_relative_geometry

from .control import ControllerConfig, DitherConfig, LEOPController
from .data import load_forest_passes, load_gps_reference
from .estimation.ukf import UKFConfig
...
def _run_simulation(mode: MeasurementMode, args) -> dict:
    ...
    if result.final_offset_s is not None:
        for epoch, state in zip(times, truth, strict=True):
            truth_geometry = relative_geometry_from_state(station, epoch, state[:3], state[3:])
            if truth_geometry.elevation_rad <= 0.0:
                continue
            predicted = tle_relative_geometry(tle, station, epoch, result.final_offset_s)
...
def simulate_command(args) -> int:
    ...
    return (
        0
        if all(row["healthy"] and row["max_visible_position_error_km"] <= 10.0 for row in results)
        else 1
    )
...
def _method_summary(rows: list[dict], label: str) -> tuple[float, float, float, int]:
    ...
    improved = sum(
        row[f"in_pass_{label}"]["median_km"] < row["in_pass_prior"]["median_km"] for row in eligible
    )
    if not values:
...
def _write_ukf_replay_markdown(rows: list[dict], path: Path) -> None:
    ...
    for forecast_index in range(5):
        ...
        prior = np.median([entry["prior"]["median_km"] for entry in entries])
        ukf = np.median([entry["ukf"]["median_km"] for entry in entries])
        improved = sum(entry["ukf"]["median_km"] < entry["prior"]["median_km"] for entry in entries)
        lines.append(
...
def replay_command(args) -> int:
    from .validation.replay import replay_pass
...
def inventory_command(args) -> int:
    from .validation.inventory import forest_contact_inventory
    ...
    for satellite_number in args.satellites:
        ...
        telemetry = data_dir / f"forest{satellite_number}.parquet"
        gps = load_gps_reference(raw_dir, satellite)
        passes = {item.contact_id: item for item in load_forest_passes(telemetry, satellite)}
...
def _write_window_simulation_summary(payload: dict, path: Path) -> None:
    ...
    keys = sorted({(row["truth_tier"], row["channel_mode"], row["estimator"]) for row in rows})
    for tier, mode, estimator in keys:
        group = [
            row
            for row in rows
            if (row["truth_tier"], row["channel_mode"], row["estimator"]) == (tier, mode, estimator)
        ]
    ...
    for tier in sorted({row["truth_tier"] for row in rows}):
        for estimator in sorted({row["estimator"] for row in rows}):
            subset = [
                row for row in rows if row["truth_tier"] == tier and row["estimator"] == estimator
            ]
            by_trial = {}
            for row in subset:
...
def simulate_windows_command(args) -> int:
    from .validation.window_simulation import (
...
def model_ablation_command(args) -> int:
    from .validation.model_ablation import run_model_ablation
...
def build_parser() -> argparse.ArgumentParser:
    ...
    replay = subparsers.add_parser(
        "legacy-replay-forest", help="reproduce the historical FOREST/BESTXYZ study"
    )
    replay.add_argument("--data-dir", required=True)
    replay.add_argument("--raw-gps-dir", required=True)
    replay.add_argument("--satellites", nargs="+", required=True)
    ...
    inventory = subparsers.add_parser(
        "legacy-inventory-forest", help="inventory the historical FOREST dataset"
    )
    inventory.add_argument("--data-dir", required=True)
    inventory.add_argument("--raw-gps-dir", required=True)
    inventory.add_argument("--satellites", nargs="+", required=True)
    ...
    windows = subparsers.add_parser(
        "legacy-simulate-forest-windows",
        help="run paired simulations on historical FOREST pass windows",
    )
    windows.add_argument("--data-dir", required=True)
    windows.add_argument("--satellites", nargs="+", required=True)
    ...
    windows.add_argument("--calibration-satellites", nargs="+", default=[])
