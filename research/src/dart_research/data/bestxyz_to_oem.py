"""Convert filtered BESTXYZ research truth to CCSDS OEM 2.0 KVN."""
...
def convert_bestxyz_to_oem(raw_dir: Path, object_name: str, output: Path) -> None:
    ...
    for epoch_s, position, velocity in zip(
        reference.utc_s, reference.position_itrf_m, velocity_itrf_m_s, strict=True
    ):
        epoch = (
            sk.time.from_unixtime(float(epoch_s)).as_datetime().isoformat().replace("+00:00", "Z")
        )
