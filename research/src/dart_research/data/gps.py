"""Research NovAtel BESTXYZ loader using receiver measurement time."""
...
def gps_sow_to_utc(
    ...
    *,
    ...
    week = np.rint((packet + gps_utc_offset_s - GPS_EPOCH_UNIX_S - seconds_of_week) / GPS_WEEK_S)
...
def _measurement_epoch_map(raw_dir: Path, satellite: str) -> dict[int, float]:
    ...
    return dict(zip(packet_ms.tolist(), utc_s.tolist(), strict=True))
...
def load_gps_reference(
    ...
    *,
    ...
    names = list(rows[0])
    position_columns = [
        name for name in names if name.endswith((".1", ".2", ".3")) and "pos_f" in name
    ]
    ...
    def order_key(name: str) -> tuple[bool, bool, bool]:
        return "_x_" not in name, "_y_" not in name, "_z_" not in name
