...
def apply_rules(document: dict[str, object], path: Path, rules: dict[str, object]) -> None:
    ...
    repair_measurement_component(definitions)
...
def repair_measurement_component(definitions: dict[str, object]) -> None:
    """Recover the base object if a code generator previously expanded its allOf.

    This makes the overlay script resilient to an interrupted schema-refresh
    attempt.  Normal reviewed documents already have a complete Measurement
    object and pass through unchanged.
    """

    measurement = definitions.get("Measurement")
    if not isinstance(measurement, dict) or isinstance(measurement.get("properties"), dict):
        return
    replacement: dict[str, object] | None = None
    for name, value in definitions.items():
        if not name.startswith("Measurement") or name == "Measurement":
            continue
        if not isinstance(value, dict):
            continue
    ...
    if replacement is None:
        raise ValueError("Measurement has no recoverable structural object")
    replacement["title"] = "Measurement"
    definitions["Measurement"] = replacement
    for name in tuple(definitions):
        if name.startswith("Measurement") and name != "Measurement":
