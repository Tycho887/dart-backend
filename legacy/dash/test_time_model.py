...
from lib.utils import Data, Config
from lib.time_model import solve_time_shift, simulate_doppler_curve
...
if __name__ == "__main__":
    ...
    time_array = np.array([iss.epoch + sk.duration(minutes=x) for x in np.linspace(0, 300, 2000)])

    fc_GHz = 2.0

    # True state: time_shift (s), bias (Hz), center frequency (GHz)
    hidden_x = np.array([-10.0, 1e4, fc_GHz])
    ...
    temp_data = Data(
        ...
        spacecraft_name="STARLINK-30477",
        contact_ids=np.full(len(time_array), "simulation"),
    )

    doppler_clean, pointing_clean = simulate_doppler_curve(hidden_x, temp_data, fc_GHz)

    obs_doppler = doppler_clean + np.random.normal(scale=500.0, size=len(time_array)) \
                  + np.random.laplace(scale=3000.0, size=len(time_array))
    ...
    obs_data = Data(
        ...
        spacecraft_name="STARLINK-30477",
        contact_ids=np.full(np.count_nonzero(dropout_mask), "simulation"),
    )

    # Configure solver with automatic model selection
    config = Config(
