...
import requests
from lib.utils import _safe_float, _safe_str, _join_field, _join_list, _iso_to_unix
...
def contact_url(contact_id: str) -> str:
    return f"{KOGS_API_BASE_URL}/contacts/{contact_id}"

def ephemeris_url(ephemeris_id: str) -> str:
    return f"{KOGS_API_BASE_URL}/ephemeris/{ephemeris_id}"

def get_contact_response(auth: str, contact_id: str) -> requests.Response:
    """Return the raw contact response for callers that need HTTP diagnostics."""
    return requests.get(
...
def get_ephemeris_response(auth: str, ephemeris_id: str) -> requests.Response:
    """Return the raw ephemeris response for callers that need HTTP diagnostics."""
    return requests.get(
...
def get_contact(auth: str, contact_id: str) -> dict:
    ...
    return validate(get_contact_response(auth, contact_id))
...
def get_spacecraft(auth: str, spacecraft_id: str) -> dict:
    ...
    return validate(requests.get(url, headers=generate_auth_header(auth), timeout=REQUEST_TIMEOUT_SECONDS))
...
def get_TLE(auth: str, ephemeris_id: str) -> dict:
    ...
    return validate(get_ephemeris_response(auth, ephemeris_id))
