...
def precompute_ground_station(
    ...
    # rotation of a single vector across all time steps
    stn_pGCRF = np.einsum("nij,j->ni", r_itrf_to_gcrf, p_itrf)

    # cross product for Earth rotation velocity
...
def range_range_model(
    ...
    # Fully transformation using Einstein summation
...
def doppler_model(
    ...
    rr, pred_pointing = range_range_model(
...
def solve_mean_elements(data: Data, config: Config, line1: str, line2: str, fc_GHz: float) -> Result:
    ...
    logger.debug(300, f"Bounds: M0 [{lb_mo:.4f}, {ub_mo:.4f}], n [{sat.no_kozai - 0.002:.6f}, {sat.no_kozai + 0.002:.6f}]")
