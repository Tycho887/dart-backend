...
logger = ProcessLogger("time_model")
...
def qmc_initialize_time_model(
    ...
    logger.info(200, f"QMC ({n_active}-param) best cost: {best_cost:.4e}")
...
def select_best_time_model(
    ...
    for k, name, active_mask in models:
        ...
        logger.info(201, f"Evaluated {name} | SSR: {ssr:.2e} | {config.criterion}: {score:.2f}")
    ...
    logger.info(202, f"Selected Model: {best_name}")
    ...
    try:
        ...
        logger.warning(400, "Singular Hessian. Using identity for covariance.")
...
def solve_time_shift(data: Data, config: Config, fc_GHz: float = 2.0) -> Result:
    ...
    logger.info(203, f"Solving for data: {data}")
    ...
    logger.info(204, f"Result is: {full_x}")

    return Result(
        x=full_x,
        cov=cov,
        name=data.spacecraft_name,
        contact_id=str(data.contact_ids[0]),
        ssr=ssr,
