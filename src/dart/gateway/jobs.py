...
from .selection import validate_selection_score
...
def _schema_sql() -> str:
    source_schema = Path(__file__).resolve().parents[3] / "migrations" / "001_processing_runs.sql"
    if source_schema.is_file():
        return source_schema.read_text()
    return files("dart.gateway").joinpath("schema.sql").read_text()
