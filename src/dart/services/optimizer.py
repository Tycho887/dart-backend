"""Transport-independent explicit production optimizer operations."""
...
from ..estimation.mean_elements import (
    MeanElementsTwoParameterConfig,
    fit_mean_elements_two_parameter,
)
from ..estimation.time_offset import (
    DopplerFit,
    DopplerSample,
    fit_time_offset,
...
def _residual_records(
    ...
    for index, measurement in enumerate(request.measurements):
        ...
        if measurement.phase_difference_rad is not None:
            channels.append(
                ObservableResidual(channel=ObservableChannel.PHASE_DIFFERENCE, consumed=False)
            )
