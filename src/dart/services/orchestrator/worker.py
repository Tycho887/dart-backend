...
from .acquisition import AdxKogsProvider, ProviderFailure
...
class SolverFailure(ServiceFailure):
    pass
...
class HttpServiceClient:
    ...
    def from_environment(cls) -> HttpServiceClient:
        ...
        return cls(
            bearer_token=token,
            solver_url=os.getenv("DART_SOLVER_URL", "http://solver:8001").rstrip("/"),
    ...
    def solve(self, request: BatchRequest) -> BatchResult:
        response = self._post(
            f"{self.solver_url}/v0/solve/batch",
        ...
        try:
            ...
            raise SolverFailure(
                "solver returned an invalid BatchResult",
                retryable=False,
            ) from exc
    ...
    def _post(
        ...
        url: str,
        payload: bytes,
        failure_type: type[SolverFailure] | type[PostprocessingFailure],
    ) -> bytes:
        request = Request(
            url,
            data=payload,
    ...
    if result.model is not candidate.metaparameters.model:
        jobs.store_candidate_error(
            ...
            StageError(
                ...
                message="solver result model did not match the candidate model",
            ),
        )
        return None
    try:
...
def _postprocess_candidate(
    ...
    try:
        ...
        return
    try:
        jobs.validate_selection_score(result, quality, claimed.request.selection_criterion)
    except jobs.SelectionValidationError as exc:
        jobs.store_candidate_error(
            claimed,
            candidate.candidate_id,
            _stage_error(PipelineStage.POSTPROCESSING, exc),
...
def _retry_global_stage(
    claimed: jobs.ClaimedRun,
    stage: PipelineStage,
    exc: AcquisitionFailure | SolverFailure | PostprocessingFailure | jobs.ArtifactConflictError,
) -> None:
    try:
...
def main() -> None:
    """Run the durable worker loop as a separate orchestrator role."""
