...
from ...frames import Station, station_state_gcrf, teme_state_to_gcrf, tle_states_teme
...
class TLEContext:
    """Orbit, station, and carrier inputs shared by Doppler models."""
...
@dataclass(frozen=True, slots=True)
class RFObservation:
    """One production Doppler sample at its measurement epoch."""
...
class BatchMeasurementCache:
    """Frame operations that depend on sample epoch but not solver parameters."""
...
def prepare_batch_cache(
    ...
    """Cache frame operations shared by all mean-element candidates."""
    ...
    for epoch in epochs:
        ...
        velocity_columns = []
        for column in basis:
            transformed_position, transformed_velocity = teme_state_to_gcrf(
                epoch, column, np.zeros(3)
            )
            position_columns.append(transformed_position)
            velocity_from_position_columns.append(transformed_velocity)
            _, transformed_velocity_basis = teme_state_to_gcrf(epoch, np.zeros(3), column)
...
def predict_cached_tle_doppler_hz(
    ...
    """Predict unbiased Doppler from a TLE at cached sample epochs."""

    position_teme, velocity_teme = tle_states_teme(context.tle, list(cache.epochs))
    satellite_position = np.einsum("nij,nj->ni", cache.teme_position_to_gcrf, position_teme)
    satellite_velocity = np.einsum(
        "nij,nj->ni", cache.teme_velocity_to_gcrf, velocity_teme
    ) + np.einsum("nij,nj->ni", cache.teme_position_velocity_term, position_teme)
