"""FOREST17 LEOP contact inventory, extracted from doppler_parquet/forest17.parquet.

May 3–4, 2026 UTC. The recovered EPHEMERIS_ID and committed GPS OEM are
the defaults. Explicit overrides remain supported; contact ephemerides never select the prior.
"""

from pathlib import Path

SPACECRAFT_ID = "81515c13-627d-4a0a-bc94-50ec0e9231cd"
CENTER_FREQUENCY_HZ = 2_216_300_000.0
CONTACT_IDS = (
    "c12b9960-e925-4286-9cd1-6a887d4e8289",
    "75d4144d-d3fc-4e18-8f14-b17da65092cb",
    "118e1a7d-4ae2-4982-a40b-7d6eb0ead2e4",
    "dc06b459-269d-446a-97ef-9b6febcd8679",
    "0a2e4679-2976-4bed-bd7a-ffd4f45e0a60",
    "6569e3a1-7aa1-4ff3-a2c0-cd1cd88fd85a",
    "4ef3b909-6d7f-460d-bf48-41aed65abf0a",
    "5233c319-41fd-4896-847f-112ddfe9c56d",
    "74df0163-cf66-41f9-b960-409fca756c93",
    "2610bd5f-3a3f-4756-be38-d31392d4324c",
    "36c92afa-8853-4630-a788-ad2e63912d51",
    "d61bfc77-1d1c-4e82-91fd-6e7489e73cb3",
    "d623db96-56fd-4783-9f1e-bf3933f4dbd8",
    "01448cbd-3659-4184-89c2-7883e4a3c6c7",
    "9303555b-beda-4b39-ac33-b6df9a7e9952",
)
EPHEMERIS_ID = "a79ad36d-0ea1-4b6a-9309-3cb603d0ae6e"

REFERENCE_OBJECT_ID = "FOREST-17"
DEFAULT_REFERENCE_OEM = (
    Path(__file__).resolve().parents[2]
    / "reports/forest-gps/20260504/FOREST-17/FOREST-17.oem"
)

DOPPLER_PARQUET = (
    Path(__file__).resolve().parents[2] / "doppler_parquet/forest17.parquet"
)
RAW_GPS_DIRECTORY = Path(__file__).resolve().parents[2] / "gps-examples"
