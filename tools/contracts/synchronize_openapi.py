...
def referenced_schema_names(value: object) -> set[str]:
    """Collect local component-schema references from a JSON value."""
    ...
    if isinstance(value, dict):
        ...
        for nested in value.values():
            result.update(referenced_schema_names(nested))
    elif isinstance(value, list):
        for nested in value:
            result.update(referenced_schema_names(nested))
    return result
...
def reachable_schemas(
    ...
    """Return exactly the schemas transitively referenced by service paths."""

    pending = list(referenced_schema_names(document.get("paths")))
    result: dict[str, object] = {}
    while pending:
        name = pending.pop()
        if name in result:
            continue
        schema = custom.get(name, shared.get(name))
        if schema is None:
            raise ValueError(f"referenced OpenAPI schema {name!r} is not canonical or custom")
        result[name] = deepcopy(schema)
        pending.extend(referenced_schema_names(schema) - result.keys())
...
def synchronized_document(path: Path, common_schemas: dict[str, object]) -> dict[str, object]:
    ...
    components["schemas"] = reachable_schemas(document, shared, custom)
    return document
